...
<tr>
	...
	<td>预计时间</td>
	<td>完成时间</td>
	<td><input id="btn2" type="button" value="批量删除" onclick="delAllWork();"/></td>
	<td><a href="addWork.jsp"><input id="btn2" type="button" value="增 加" /></a></td>
</tr>
...
%>		
<tr>
	<td><input type="checkbox" name="check" value=<%= w.getwId() %>></td>
	<td><%= w.getwId() %></td>
	<td><%= w.getwName() %></td>
	<td><%= w.geteTime() %></td>
	<td><%= w.getaTime() %></td>
		...
		<td><a href="updateWork?f=del&wid=<%=w.getwId()  %>
		"onclick="return confirm('您确定要删除该条记录嘛？');">删 除</a></td>
		
		<td><a href="updateWork.jsp?wid=<%=w.getwId()  %>">修 改</a>		
	</td>
</tr>
<%		
		//}
