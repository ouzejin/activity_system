...
<tr>
	...
	<td>预计花费</td>
	<td>实际花费</td>
	<td><input id="btn2" type="button" value="批量删除" onclick="delAllBudget();"/></td>
	<td><a href="addBudget.jsp"><input id="btn2" type="button" value="增 加" /></a></td>
...
%>	
<!-- <td>
		<a href="addBudget.jsp?wid=<%=b.getwId()  %>">增加</a></td> -->	
<tr>
	<td><input type="checkbox" name="check" value=<%=b.getwId() %>></td>
	...
	<td><%= b.getaCost() %></td>
		...
		<td><a href="updateBudget?f=del&mid=<%=b.getmId() %>&wid=<%= b.getwId() %>"
		"onclick="return confirm('您确定要删除该条记录嘛？');">删 除</a></td>
		
		<td><a href="updateBudget.jsp?wid=<%=b.getwId()%>&mid=<%=b.getmId()%>">修 改</a>					
	</td>
</tr>
<%		
		//}
